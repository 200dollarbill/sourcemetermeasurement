...
def main():
    ...
    # 3. Calculate Implied Gauss, then correlate Resistance against it (R/G)
    idx_0 = np.argmin(np.abs(on_I))
    R_nominal = on_R[idx_0]
    delta_R = on_R - R_nominal
    implied_G = -(delta_R / sens)

    # Fit Resistance (raw) vs Implied Field -> slope is Ohms/G
    p = np.polyfit(implied_G, on_R, 1)
    slope = p[0]  # Ohms per Gauss

    # ==========================================
    # PLOTTING THE INFOGRAPHIC
    # ==========================================
    fig, axs = plt.subplots(1, 3, figsize=(18, 6), facecolor=(0.94, 0.94, 0.94))
    fig.suptitle('How the Correlation (R/G) is Calculated', fontsize=24, fontweight='bold', y=1.05)
    ...
    # --- PANEL 3: Resistance vs Implied Field (The R/G Result) ---
    axs[2].plot(implied_G, on_R, 'b-', linewidth=2.5, label='Sweep Data')

    # Plot the line of best fit to visualize the R/G slope
    fit_line = np.poly1d(p)
    axs[2].plot(implied_G, fit_line(implied_G), 'r--', linewidth=3.0, label='Best Fit Line')

    style_ax(axs[2], 'Step 3: Resistance vs Implied Field', 'Implied Magnetic Field (G)', 'Resistance (Ohms)')

    final_text = (
        "Implied Field = Δ Resistance / Sensitivity\n"
        f"Best fit line (red) & Resistance (blue)\n"
        f"gives the correlation R/G = {slope:.3f} Ohms/G"
    )
    ...
    output_path = os.path.join(base_dir, 'RG_Correlation_Visualization.png')
