function Tektronix2400_GUI()
    % Tektronix 2400 Resistance Measurement GUI
    % Initializes the device via GPIB (address 24), reads resistance, and plots it.
    
    % Create UI Figure
    fig = uifigure('Name', 'Tektronix 2400 Resistance Measurement', 'Position', [100, 100, 800, 500], 'Color', 'w');
    
    % Connection Panel
    pnl_conn = uipanel(fig, 'Title', 'Connection', 'Position', [20, 380, 220, 100], 'BackgroundColor', 'w', 'FontWeight', 'bold');
    ...
    function connectHW(~, ~)
        try
            ...
            % Create VISA-GPIB object for the SourceMeter
            % Based on TestProgram.m (Address 24)
